#!/usr/bin/env python3
"""Plot baseline success rate vs inference steps K with binomial CI."""
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="Sweep CSV with columns method, k, success, num_episodes.",
    )
    ap.add_argument(
        "--method",
        default="baseline",
        help="Value in the method column to plot.",
    )
    ap.add_argument(
        "--confidence",
        type=float,
        nargs="+",
        default=[0.995, 0.95],
        help="Central CI levels, widest first in legend (default: 99.5%% 95%%).",
    )
    ap.add_argument(
        "-o",
        "--output",
        type=Path,
        default=repo_root / "results/figures/baseline_sr_vs_k.png",
    )
    ap.add_argument(
        "--title",
        default="Baseline success rate vs inference steps",
    )
    args = ap.parse_args()

    confidences = sorted(set(args.confidence), reverse=True)
    rows = load_sweep_rows(args.csv.resolve(), args.method)

    ks = np.array([r["k"] for r in rows])
    rates = np.array([r["rate"] for r in rows])

    ci_bands: list[tuple[float, np.ndarray, np.ndarray]] = []
    for conf in confidences:
        ...
        ci_bands.append((conf, lo, hi))

    fig, ax = plt.subplots(figsize=(8, 5))
    color = "#2563eb"
    fill_alpha = {0.995: 0.12, 0.99: 0.15, 0.95: 0.22}

    for conf, lo, hi in ci_bands:
        ...
        ax.fill_between(
            ...
            label=f"{label_pct} CI (Clopper–Pearson)",
        )
    ...
    ax.set_xticks(ks)
    hi_max = max(hi.max() for _, _, hi in ci_bands)
    ax.set_ylim(0, min(1.0, hi_max + 0.08))
    ...
    ax.set_title(
        f"{args.title}\n"
        f"method={args.method}, n={n_ep} episodes/point, seed=5678",
        fontsize=11,
    )
