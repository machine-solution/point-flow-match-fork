...
# Dexter submission wrapper for MeanFlow Transformer checkpoint validation.
# Usage:
#   CKPT_NAME=<run_name> bash bash/dexter_validate_meanflow_transformer.sh
...
SBATCH_FILE="${SLURM_DIR}/dexter_validate_meanflow_transformer_${CKPT_NAME}_$(date +%Y%m%d_%H%M%S).sbatch"

cat > "${SBATCH_FILE}" <<EOF
#!/usr/bin/env bash
#SBATCH --job-name=mftr_val_${CKPT_NAME}
