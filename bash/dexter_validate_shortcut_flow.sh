...
# Dexter submission wrapper for Shortcut PointFlowMatch checkpoint validation.
# Usage:
#   CKPT_NAME=<run_name> bash bash/dexter_validate_shortcut_flow.sh
...
SBATCH_FILE="${SLURM_DIR}/dexter_validate_shortcut_${CKPT_NAME}_$(date +%Y%m%d_%H%M%S).sbatch"

cat > "${SBATCH_FILE}" <<EOF
#!/usr/bin/env bash
#SBATCH --job-name=sc_val_${CKPT_NAME}
